...
object JDBCLib {
  ...
  def main(args: Array[String]) {
    ...
    try {
//      val rs1 = selectQuery("SELECT * FROM user")
//      processResults(rs1, Array("first_name"))
//
//      println()
//
      // getString ALL example
      val rs2 = selectQuery("SELECT * FROM user")
      processResults(rs2, (col: String) => println(col))

//      updateQuery("DELETE FROM user WHERE user_id = 4")

//      updateQuery("ALTER TABLE user_addr " +
//        "  ADD CONSTRAINT fk_user " +
//        "  FOREIGN KEY(uid) " +
//        "  REFERENCES user(user_id) ")

//      updateQuery("ALTER TABLE user_addr DROP FOREIGN KEY fk_user")
    }
    finally {
      conn.close()
    }
